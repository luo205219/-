...
powering_up = False
potted_ball = False
potted_balls_player1 = []
potted_balls_player2 = []
current_player = 1  # 1 表示玩家一，2 表示玩家二
shot_taken = False  # 标记是否击球
player_switched_this_turn = False  # 标记本次击球后玩家是否已切换
new_turn = True  # 标记是否为新的一轮

# 加载音效
cue_hit_sound = pygame.mixer.Sound("sounds/cue_hit.mp3")
# 定义颜色
BG = (50, 50, 50)
RED = (255, 0, 0)
WHITE = (255, 255, 255)
...
large_font = pygame.font.SysFont("Microsoft YaHei", 100)
SCREEN_WIDTH = 1200
SCREEN_HEIGHT = 672
BOTTOM_PAMEL = 50
...
pygame.display.set_caption("台球游戏-双人模式")
# pymunk空间
space = pymunk.Space()
# 创建静态体
    ...
    def draw(self, surface):
        ...
        if powering_up:
            for b in range(math.ceil(self.force / 1500)):
                surface.blit(
                    self.power_bar,
                    (self.rect.centerx - 30 + (b * 15), self.rect.centery + 30),
                )
...
class Ball:
    ...
    def draw(self, surface):
        ...
        surface.blit(
            self.image,
            (self.body.position[0] - self.radius, self.body.position[1] - self.radius),
        )
...
# 创建球洞
pockets = [(35, 34), (599, 34), (1163, 34), (35, 632), (599, 632), (1163, 632)]

# 创建台球缓冲区域
cushions = [
    [(55, 32), (86, 55), (536, 55), (562, 32)],
...
while run:
    ...
    for ball in balls:
        for pocket in pockets:
            if ball.is_potted(pocket, pocket_dia):
                if ball == cue_ball:
                    ball.reset_position((888, SCREEN_HEIGHT / 2))  # 直接重置母球位置
                else:
                    space.remove(ball.body)
                    balls_to_remove.append(ball)
                    if current_player == 1:
                        potted_balls_player1.append(ball.image)
                    else:
                        potted_balls_player2.append(ball.image)
                    potted_this_turn = True
                    change_time = 0
    ...
    # 如果所有球静止，允许击球
    all_stationary = all(ball.is_stationary() for ball in balls)
    if all_stationary:
        taking_shot = True
        if shot_taken:
            if not potted_this_turn:
    ...
    if powering_up and game_running:
        ...
        cue.reset_force()
        shot_taken = True
        player_switched_this_turn = False
        new_turn = False

    # 绘制当前玩家
    player_text = f"玩家 {'一' if current_player == 1 else '二'}"
    draw_text(player_text, font, RED, SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT + 10)

    # 画进入球洞的球体
    for i, ball in enumerate(potted_balls_player1):
        screen.blit(ball, (10 + (i * 50), SCREEN_HEIGHT + 10))
    for i, ball in enumerate(potted_balls_player2):
        screen.blit(ball, (SCREEN_WIDTH - 60 - (i * 50), SCREEN_HEIGHT + 10))

    # 检查所有的球体是否都进了球洞
    if len(balls) == 1:
        if len(potted_balls_player1) > len(potted_balls_player2):
            winner_text = "玩家一胜利！"
        elif len(potted_balls_player1) < len(potted_balls_player2):
            winner_text = "玩家二胜利！"
        else:
            winner_text = "平局！"
        draw_text(
